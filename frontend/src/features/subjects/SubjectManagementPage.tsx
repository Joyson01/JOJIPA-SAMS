...
export const SubjectManagementPage: React.FC = () => {
  ...
  const loadTimetable = useCallback(async () => {
    ...
    try {
      const data = await fetchAllTimetableEntries(undefined, selectedDay === 'ALL' ? undefined : selectedDay);
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between bg-white p-3.5 rounded-2xl border border-slate-200 shadow-sm text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-800">Schedule: Class TE-B (Effective: 15/06/2026)</span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${
                  selectedDay === 'ALL' ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
                All Days
              </button>
          ...
          <div className="space-y-4">
            {loadingTimetable ? (
              <div className="bg-white rounded-2xl p-12 text-center text-slate-400 text-xs flex items-center justify-center gap-2 border border-slate-200">
                <RefreshCw className="w-4 h-4 animate-spin text-blue-600" />
                    ...
                    <div key={day} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-3">
                      <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                        ...
                        <span className="text-[11px] text-slate-400 font-medium font-mono">
                          {dayEntries.length} Scheduled Slots
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2.5">
                        {dayEntries.map((entry) => (
                          <div
                            key={entry.id}
                            className={`p-3 rounded-xl border transition flex flex-col justify-between text-xs ${
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between text-[10px] text-slate-500 pt-2 border-t border-slate-200/50 mt-2 font-mono">
                              <span>{entry.room ? `Room: ${entry.room}` : 'Classroom'}</span>
                              {entry.batch && <span className="font-bold text-blue-600">Batch {entry.batch}</span>}
                            </div>
                          </div>
                        ))}
                      </div>

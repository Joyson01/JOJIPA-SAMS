...
def main() -> None:
    ...
    while True:
        ...
        if frame_count % max(1, args.frame_skip) == 0 or not cached_face_results:
            # 65% scale balances speed with small/partially covered face recall
            scale = 0.65
            ...
            for face in faces:
                ...
                # Two-tier recognition logic for full vs partially obstructed faces
                if best_eid is not None and best_sim >= args.threshold:
                    # High confidence full match (Green)
                    ...
                    # Obstructed / Partial match (Masks, sunglasses, angles - Yellow)
                    name = student_names.get(best_eid, best_eid)
